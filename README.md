...
- **Nhập bằng bàn phím (nhanh nhất)**:
  - `Tab` chuyển sang trường kế tiếp.
  - Ở ô lựa chọn: bấm `1`/`2`/`3` để chọn, hoặc `C` = Có / `K` = Không — **tự nhảy sang trường kế tiếp**.
  - `←`/`→` đổi lựa chọn (không nhảy trường); `Enter` sang ô tiếp theo.
  - Nhập cả phiếu khám gần như không cần rời tay khỏi bàn phím.
- **Chọn sẵn giá trị "Bình thường"**: các trường khám lâm sàng / sinh tồn mặc định là *Bình thường*, chỉ cần bấm vào mục **bất thường**.
- **Nút "Tất cả bình thường" / "Tất cả Có"** cho từng mục.
- **M-CHAT-R tự tính điểm** ngay khi nhập và phân loại nguy cơ.
- **Tìm kiếm hồ sơ theo tên trẻ**; lọc theo mẫu.
- **In phiếu / lưu PDF**: nút *In / Lưu PDF* ở mỗi hồ sơ (bản in gọn theo từng mục).
- **Xuất Excel**: mỗi mẫu 1 sheet, mỗi trường 1 cột (Mẫu 9 có cột điểm & phân loại; có cột *Người nhập*).

## Đăng nhập nhiều người dùng

- Lần chạy đầu, mở app sẽ vào trang **/setup** để tạo **tài khoản quản trị** đầu tiên.
- Sau đó mọi trang đều yêu cầu **đăng nhập** (`/login`).
- Quản trị viên vào **Người dùng** (`/admin/users`) để thêm/xóa nhân viên (vai trò *Quản trị* hoặc *Nhân viên*).
- Mọi người đăng nhập đều xem được tất cả hồ sơ; mỗi hồ sơ ghi nhận **người nhập**.
- Mật khẩu được băm bằng bcrypt; phiên đăng nhập là JWT trong cookie `httpOnly`.
...
3. Tạo file `.env.local` (copy từ `.env.example`):
   ...
   AUTH_SECRET="chuoi-bi-mat-ngau-nhien-tren-32-ky-tu"
   ```
   Tạo `AUTH_SECRET` bằng: `openssl rand -base64 32`
4. Chạy lại `npm run dev`. Các bảng `ksk_records` và `users` sẽ **tự động tạo** ở lần ghi đầu tiên.
...
3. Trong **Environment Variables**, thêm:
   ...
   - `AUTH_SECRET` = chuỗi bí mật ngẫu nhiên (≥ 32 ký tự) để ký phiên đăng nhập.
4. Bấm **Deploy**. Vercel tự build (`next build`).
5. Mở app lần đầu → trang **/setup** tạo tài khoản quản trị.
...
| created_by | TEXT | người nhập |
| created_at / updated_at | TIMESTAMPTZ | thời gian |

Bảng `users`: `id, username (unique), password_hash (bcrypt), full_name, role (admin/user), created_at`.
